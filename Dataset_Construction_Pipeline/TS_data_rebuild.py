...
if __name__ == "__main__":
    ...
    if args.sport == 'deadlift':
        ...
        lengths = merge_files(class_dir, args.output_root)
        data_check(lengths, args.output_root)
    elif args.sport == 'benchpress':
        class_dir = glob.glob(path.join(args.data_path, '*'))
        class_dir = [p for p in class_dir if path.isdir(p)]
        merge_files(class_dir, args.output_root)
    # python ./Dataset_Construction_Pipeline/TS_data_rebuild.py --sport benchpress --data_path Data/public/BenchpressDataset --output_path Data/benchpress